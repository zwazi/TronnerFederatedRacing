...
Unix sockets exist, the current map has identical bytes in the engine cache and
public mirror, and every effective catalog map exists in the public mirror with
its authenticated Firebase digest. Run it on all nodes to prove every required
direction, then compare `catalog_effective_maps` and
`catalog_effective_sha256`; both must match across the fleet. The check is
read-only and does not restart or reload anything.
...
The `PROBE` milestones record login latency, protocol-correct game-state
transitions, received players, federation ghost identity, current map settings,
and clean disconnect. An optional final argument sends one bounded chat
command. `/nextmap` is a useful non-mutating round-trip canary on a follower:
its reply proves follower engine -> follower sidecar -> leader controller ->
follower sidecar -> follower engine delivery. These are one-shot clients; do
not leave probes connected or use them as a keepalive service.
