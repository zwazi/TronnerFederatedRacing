...
Python racing server script
  ↕ Firebase catalog, live status, replays, and admin command queue
tronner.io / Vectron
```

The dedicated server owns cycle simulation and networking. The server script owns
...
Server and server script run as the unprivileged `armagetron` account under
